...
import com.github.abhishek5074.newsapp.databinding.FragmentSportsBinding
...
class SportsFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val binding : FragmentSportsBinding = DataBindingUtil.inflate(inflater,R.layout.fragment_sports,container,false)


        NewsApiService.newsApi.getCategoryNews(Credentials.API_KEY, "in", "sports",100)
            .enqueue(object :
                Callback<Model> {
                override fun onResponse(call: Call<Model>, response: Response<Model>) {
                    item.addAll(response.body()!!.articles)
                        mAdapter = NewsAdapter(item)
                        binding.sportsRecyclerView.adapter = mAdapter
        ...
        binding.sportsRecyclerView.layoutManager = LinearLayoutManager(this.activity)
        return binding.root
    }
