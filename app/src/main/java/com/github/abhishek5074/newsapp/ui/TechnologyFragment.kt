...
import com.github.abhishek5074.newsapp.databinding.FragmentTechnologyBinding
...
class TechnologyFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val binding : FragmentTechnologyBinding = DataBindingUtil.inflate(inflater,R.layout.fragment_technology,container,false)

        NewsApiService.newsApi.getCategoryNews(Credentials.API_KEY, "in", "technology",100)
            .enqueue(object :
                Callback<Model> {
                override fun onResponse(call: Call<Model>, response: Response<Model>) {
                    item.addAll(response.body()!!.articles)
                        mAdapter = NewsAdapter(item)
                        binding.technologyRecyclerView.adapter = mAdapter
        ...
        binding.technologyRecyclerView.layoutManager = LinearLayoutManager(this.activity)
        return binding.root
    }
